...
function validateResearchResponse(response, fixtureCase, requireFixture = true) {
const errors = [];
...
if (!fixtureCase && requireFixture) fail("semantic context: selected fixture case required");
const normalizedResponse = response && typeof response === "object"
  ? (Array.isArray(response) ? [...response] : {...response})
  : response;
if (normalizedResponse) {
  validateUnicode(normalizedResponse);
  if (!Object.hasOwn(normalizedResponse, "candidate_mechanisms")) normalizedResponse.candidate_mechanisms = [];
  if (normalizedResponse.research_method !== "five_lens_briefing") fail("response: five_lens_briefing required");
  const process = normalizedResponse.five_lens_process_evidence;
  ...
  if (!process || !["delegated", "sequential_fallback"].includes(lensMode) || !["delegated", "sequential_fallback"].includes(peerMode)) fail("process: supported execution modes required");
    ...
    const effectiveTier = tierResolutionValid(process.tier_resolution, normalizedResponse.tier);
    ...
    const perspectiveScan = Array.isArray(normalizedResponse.perspective_scan) ? normalizedResponse.perspective_scan : [];
    const questionTrace = Array.isArray(normalizedResponse.question_trace) ? normalizedResponse.question_trace : [];
    if (!Array.isArray(normalizedResponse.perspective_scan)) fail("perspective scan: array required");
    if (!Array.isArray(normalizedResponse.question_trace)) fail("question trace: array required");
    ...
    if (!peerInputBindingValid(binding, process, accepted, lensMode, normalizedResponse)) fail("peer review input binding: invalid evidence");
    finalArtifactsValid(normalizedResponse, accepted, binding);
    semanticContextValid(fixtureCase, process.frozen_assignment_packets, accepted, normalizedResponse, binding);
    const highStakesApplicable = fixtureCase?.semantic_context?.high_stakes_context?.applicable === true;
    synthesisValid(normalizedResponse.synthesis_briefing, highStakesApplicable, "synthesis briefing");
    synthesisValid(binding?.initial_synthesis, highStakesApplicable, "peer initial synthesis");
    if (!equal(binding?.initial_synthesis, normalizedResponse.synthesis_briefing) && ["accepted", "accepted_with_concerns"].includes(normalizedResponse.peer_review?.verdict)) fail("peer review: accepted initial/final synthesis drift");
    if (!contentDigest(process.final_synthesis_digest) || process.final_synthesis_digest !== digest(normalizedResponse.synthesis_briefing)) fail("final synthesis: digest");
    peerReviewValid(normalizedResponse.peer_review, peerMode, process, identities, rootPasses, binding, process.final_synthesis_digest, normalizedResponse.synthesis_briefing?.recommendation);
    highStakesRecommendationValid(normalizedResponse, binding, normalizedResponse.peer_review, process.frozen_assignment_packets, fixtureCase);
  }
}
return errors;
}

function runCli(responsePath, fixturePath, caseId) {
  const fs = require("fs");
  let fixtureCase;
  let response;
  const errors = [];
  try {
    ...
    if (!fixtureCase) errors.push("semantic context: selected fixture case required");
  } catch { errors.push("semantic context: readable fixture case required"); }
  try { response = JSON.parse(fs.readFileSync(responsePath, "utf8")); } catch { errors.push("response: valid JSON required"); }
  errors.push(...validateResearchResponse(response, fixtureCase, false));
  if (errors.length) {
...
if (require.main === module) runCli(process.argv[2], process.argv[3], process.argv[4]);
