...
Cover 4-8 distinct attack perspectives. For high-stakes stress tests, prefer one independent subagent per attack perspective when available. If the active tool policy requires explicit user authorization before spawning subagents, ask: `This reasoning method works best with independent stress-test subagents. May I use subagents for this stress test?`

If approved, delegate each attack perspective with only the argument, claims, constraints, and perspective brief. If denied or unavailable, run the perspectives sequentially in the main session and record that fallback.
