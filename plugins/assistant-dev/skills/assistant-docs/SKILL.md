...
Migration note: v3 adds feature-preparation completeness for ordinary and Pack-backed documentation. v2 consumers must treat `blocked_incomplete_evidence` and `blocked_incomplete_pack` as no-write recovery, and provide admissible feature-preparation evidence refs before documenting behavior claims or carried Product questions. Existing v2 behavior keeps files_updated required/non-empty for ordinary and current-Pack documentation, but permits omission for typed no-write recovery. Pack projections require non-empty boundaries and exact five-concern design-pressure coverage; v1 consumers must adapt before accepting v2.
