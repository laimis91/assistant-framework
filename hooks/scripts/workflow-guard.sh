...
if [[ "$status" == *"BUILDING"* || "$status" == *"VERIFYING"* ]]; then
    if [[ -z "$subagent_policy_state" || -z "$subagent_execution_mode" || "$subagent_policy_state" == "unknown" || "$subagent_execution_mode" == "unknown" ]]; then
        assistant_workflow_guard_block "Workflow subagent policy is unresolved for an active Build/Verify edit. Resolve subagent_policy_state and subagent_execution_mode in the task journal before editing project files. For development/code-work roles, ask once for explicit subagent authorization unless the current user prompt already authorized it."
        exit 0
    fi
    if [[ "$subagent_missing_key" == "authorization_required_unresolved" ]]; then
        assistant_workflow_guard_block "Subagent authorization is unresolved. Ask once for the needed Code Mapper/Architect/Code Writer/Builder-Tester/Reviewer delegation scope and wait before editing project files. Do not continue Build inline while authorization_required is unresolved."
        exit 0
    fi
    if [[ "$subagent_execution_mode" == "delegated" ]] && assistant_phase_requires_subagent_roles "$TASK_FILE"; then
        if ! assistant_phase_has_labeled_evidence "$TASK_FILE" "Code Writer dispatch" && assistant_phase_required_subagent_roles "$TASK_FILE" | grep -qx "Code Writer"; then
            assistant_workflow_guard_block "Delegated Build requires Code Writer dispatch evidence before project file edits are performed. Spawn the code-writer agent or switch to a valid direct_fallback reason before editing."
            exit 0
        fi
    fi
fi
