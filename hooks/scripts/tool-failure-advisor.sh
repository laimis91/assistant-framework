...
if echo "$ERROR" | grep -q "NETSDK1100"; then
...
elif echo "$ERROR" | grep -q "NETSDK1045"; then
...
elif echo "$ERROR" | grep -qE "CS0246|CS0234"; then
...
elif echo "$ERROR" | grep -q "CS1061"; then
...
# Test failures
elif echo "$ERROR" | grep -qE "Failed!|Test Run Failed"; then
...
# Permission denied
elif echo "$ERROR" | grep -qiE "permission denied|access denied|EACCES"; then
...
# Process timeout
elif echo "$ERROR" | grep -qiE "timeout|timed out"; then
...
elif echo "$ERROR" | grep -qiE "NU1100|NU1101|Unable to resolve"; then
