...
2. ORCHESTRATOR OWNERSHIP: Coordinate the work; keep discovery context mapping with code-mapper for medium+ work, project source edits and code implementation with code-writer, builds/tests with builder-tester, and independent review with reviewer when subagent delegation is authorized and available. For Codex, spawn custom agents by explicitly asking Codex to spawn the configured agent name (for example, code-mapper, code-writer, builder-tester, reviewer); a hidden/implicit subagent capability may not appear as a normal tool in the visible tool list. If the active policy requires explicit user authorization before subagents, ask once before spawning; after approval, use delegated mode. Use direct fallback only for explicit authorization_denied, subagents_unavailable after a real spawn failure, or policy_disallowed. The orchestrator may create and update framework-owned state artifacts such as .codex/task.md, .codex/context-map.md, .codex/session.md, and .codex/working-buffer.md; it does not edit project source files directly in delegated mode.
