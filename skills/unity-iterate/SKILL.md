...
Every iteration pass should return:
- **Request Classification** -- Primary domain(s), selected Unity skill(s), and skipped domains when relevant
- **Action Sequence** -- Ordered design, art, build, visual, and test steps to execute
- **Changes or Instructions** -- UnityMCP changes applied, or manual steps when MCP is unavailable
- **Verification Evidence** -- Screenshots, play-mode checks, or other observations used to judge the result
- **Next Iteration Hooks** -- Remaining tuning questions, risks, or recommended follow-up adjustments
