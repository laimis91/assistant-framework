...
| **Medium** (feature, refactor, endpoint) | Discover -> Plan -> [Design] -> Build & Test -> Review -> Document |
| **Large** (new project, multi-module) | Discover -> Plan -> Design -> Build & Test -> Review -> Document |
...
| **Architect** | `architect` | `architect` | Read-only | Plan |
...
| **Mega** | All roles, parallel Code Writers per sub-task | Mapper → Explorer → Architect → parallel Writers → Builder/Tester and Reviewer run once at integration |
...
- **Code Mapper runs first** on medium+ tasks — its output feeds into Code Writer and Architect
...
## Phase 3: Design (UI/UX only, skip for backend)
...
2. After each step: build + test, update task journal
3. If fails: fix before next step
4. Tests alongside code, not after
5. After all steps: integration tests
...
- **assistant-memory**: Capture insights at task completion (Phase 6)
