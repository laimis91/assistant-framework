...
- Configured in the active agent's MCP settings
...
Every scene-building pass should return:
- **Recipe Summary** -- Scene, prefab, GameObject, material, VFX, or UI recipe selected
- **MCP Actions** -- Ordered UnityMCP calls made, including important returned instanceIds
- **Created or Modified Objects** -- Names, hierarchy locations, components, materials, and prefab paths
- **Verification Result** -- Hierarchy checks, screenshots, play-mode state, or manual confirmation steps
- **Fallback Instructions** -- Step-by-step manual actions when UnityMCP is unavailable or incomplete
