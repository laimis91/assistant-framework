...
test_start "memory protocol source keeps lean {agent_state_dir} template paths substitutable"
if grep -Fq "{agent_state_dir}/task.md" "$FRAMEWORK_DIR/memory-protocol.md" \
    && grep -Fq "{agent_state_dir}/session.md" "$FRAMEWORK_DIR/memory-protocol.md" \
    && grep -Fq "{agent_state_dir}/working-buffer.md" "$FRAMEWORK_DIR/memory-protocol.md" \
    && grep -Fq "~/{agent_state_dir}/memory" "$FRAMEWORK_DIR/memory-protocol.md" \
    ...
    fail "memory-protocol.md must keep its compact memory/state paths agent-substitutable without literal agent directories"
fi
