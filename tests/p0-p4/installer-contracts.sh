...
p0p4_register_cleanup "$INSTALL_HOME_TWO" "$FIXTURE_FRAMEWORK"
...
REFERENCE_FIXTURE
if HOME="$INSTALL_HOME_TWO" bash "$FIXTURE_FRAMEWORK/install.sh" --agent codex --skill path-substitution-contract --no-hooks >/tmp/p0p4-install-subst.out 2>/tmp/p0p4-install-subst.err; then
    installed_skill="$INSTALL_HOME_TWO/.codex/skills/path-substitution-contract"
